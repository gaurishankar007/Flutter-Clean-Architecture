# Clean Architecture Flutter — Claude Code Rules (CLAUDE.md)

> These rules apply to Claude Code working in this workspace. Follow them strictly.
...
- **`clean-architecture-codegen`** — Runs `build_runner` code generation.
- **`clean-architecture-scaffolding`** — Uses `mason` to scaffold features and pages.
- **`clean-architecture-api-integration`** — Automates integrating a new API endpoint through the data source → repository → domain layers.

`.claude/workflows/release.md` documents how a release actually reaches
